 ...
 */
 (function(root) {
  ...
  return {
  "name": "++coffee",
  "description": "CoffeeScipt but compiles to c++",
  "keywords": [
    "c++",
    "language",
    "coffeescript",
...
(function() {
  ...
  RESERVED = ['case', 'function', 'auto', 'void', 'with', 'const', 'let', 'enum', 'native', 'implements', 'interface', 'package', 'private', 'protected', 'public', 'static'];
...
(function() {
  ...
  exports.Block = Block = (function(superClass1) {
    ...
    Block.prototype.compileRoot = function(o) {
      ...
      return [].concat(prelude, this.makeCode("//start global scope\n\n"), fragments, this.makeCode("//end global scope\n\n"));
  ...
  exports.ImportDeclaration = ImportDeclaration = (function(superClass1) {
    ...
    ImportDeclaration.prototype.compileNode = function(o) {
      ...
      o.importedSymbols = [];
      code = [];
      code.push(this.makeCode(this.tab + "#include "));
      if (this.clause != null) {
  ...
  exports.Assign = Assign = (function(superClass1) {
    ...
    Assign.prototype.compileNode = function(o) {
      ...
      if (!this.context) {
        ...
        if (!(typeof varBase.hasProperties === "function" ? varBase.hasProperties() : void 0)) {
          if (this.moduleDeclaration) {
            ...
            o.scope.add(varBase.value, 'auto');
          } else {
            this.checkAssignability(o, varBase);
      ...
      //changed: for ++coffee
      compiledName[0].code = 'auto ' + compiledName[0].code; 
    ...
    For.prototype.compileNode = function(o) {
      ...
      if (this.range) {
        ...
        if (!this.object && !this.from) {
          ...
          declare = "auto " + kvarAssign + ivar + " = " + svar + ".begin()"; 
          declareDown = "" + kvarAssign + ivar + " = " + svar + ".length - 1";
          
          compare = ivar + " != " + svar + ".end()"
          //compare = ivar + " < " + lvar;
          compareDown = ivar + " >= 0";
...
let parseArgs = function(ag){
	while (ag.length > 0) {
		...
		if (com1 == null) {
			if (com2 != null){
				if (com2 == '-h') {
					writeln(ccoffee.programName + ' [-h|-v] -c file -o executable');
          exit();
				}
				else if (com2 == '-v') {
					writeln(ccoffee.programName + ' version 0.1 using coffeescipt version 1.0, compiling with g++ std=20');
          exit();
				}
			}
		}
			...
			if ((flags & fileCheck) != 0) {
				writeln('Error: cannot compile multiple files (Yet)');
        exit();  
				//writeln(com1 + ' ' + com2);
				break;
			}
			
			var res = system.exists(com2); 
			if (!res) {
				writeln('unable to open file: ' + com2); 
				exit();  
			}
      ccoffee.hasFile = true; 
      ccoffee.inputFile = com2; 
			flags |= fileCheck; 
		} else if ( com1 == '-o') {
			
			if ((flags & createExecutable) != 0) {
				writeln('Error: multiple output files '); 
				exit(); 
			}
			
			writeln('Output file: ' + com2);
			ccoffee.outputFile = com2; 
			flags |= createExecutable; 
		} else if (com1 == '-entry') {
      //writeln('entry file: ' + com2); 
      ccoffee.hasMain = true; 
      ccoffee.entry = com2;
    }
...
if (system.arguments.length !== 0) {
  parseArgs(system.arguments); 
} else {
  writeln('no input file specified');
}

header = '//' + ccoffee.inputFile.toUpperCase().replace('.', '_') + '\n#include <iostream>\n\nusing namespace std;\n\n'; 
header += '#define var //\n'
footer = '\n#endif //' + ccoffee.inputFile.toUpperCase().replace('.', '_');
if (ccoffee.hasFile) {
  try{

    
    writeln('Reading: ' + ccoffee.inputFile);
    var src = new Stream(ccoffee.inputFile); 
    var sourceCode = src.readFile(); 
    
    
    var result = CoffeeScript.compile(sourceCode, {}); 

    result = header + result; 

    if (ccoffee.outputFile === 'tmp' || ccoffee.outputFile === '') ccoffee.outputFile = ccoffee.inputFile; 

    var bytes; 
    if (ccoffee.hasMain) { 
      var main = '\nint main(int argc, char **argv) {\n\t' + ccoffee.entry + "(&argc, &argv);\n}\n"; 
      writeln('Writing: ' + ccoffee.outputFile+'.cpp');
      var bytes = new Stream(ccoffee.outputFile+'.cpp', 'w').write(result + main);   
      writeln('wrote ' + bytes + ' bytes to ' + ccoffee.outputFile+'.cpp');
   
    } else {
      writeln('Writing: ' + ccoffee.outputFile+'.h');
      var bytes = new Stream(ccoffee.outputFile+'.h', 'w').write(result);   
      writeln('wrote ' + bytes + ' bytes to ' + ccoffee.outputFile+'.h');
      
    }

    var createExe = false; 

    var l = ccoffee.outputFile.length; 
    var ops = ccoffee.outputFile[l - 4] + ccoffee.outputFile[l - 3] + ccoffee.outputFile[l - 2] + ccoffee.outputFile[l - 1]; 

    if (ops == '.exe') {
      var compiler = 'g++'; 
      var args = ' -c ' + ccoffee.outputFile+'.cpp' + ' -o '+ ccoffee.outputFile; 
      var dir = system.cwd; 

      writeln(compiler + args )
      //system.execute( compiler, args, dir);
      //var proc = new Process(compiler, args);

    }

    //var params = '-c ' +ccoffee.outputFile + '.cpp ' + ' -o ' + ccoffee.outputFile + '.exe'; 
    //var command = 'gcc';  
    //system.execute(command, params, dir);
    
    exit();
  }catch (e){
    writeln(e);
    writeln('Compiler failed');
    exit();
  }
} 
